...
class FreeFire(object):
    ...
    def aimneck():
        ...
        neck_scan = bytes.fromhex('53 68 61 64 6F 77 49 6E 74 65 6E 73 69 79 62 6F 6E 65 5F 48 69 70 73')
        neck_replace = bytes.fromhex('53 68 61 64 6F 77 49 6E 74 65 6E 73 69 79 62 6F 6E 65 5F 4E 65 63 6B')
    ...
    @staticmethod
    def norecoil():
        scan = bytes.fromhex('00 0A 81 EE 10 0A 10 EE 10 8C BD E8 00 00 7A 44 F0 48 2D E9 10 B0 8D E2 02 8B 2D ED')
        replace = bytes.fromhex('00 0A 81 EE 10 0A 10 EE 10 8C BD E8 00 00 00 00 F0 48 2D E9 10 B0 8D E2 02 8B 2D ED')
    ...
    def clear_reports():
        ...
        try:
            flag = find_replace_bytes(scan, replace)
            show_notification('Clear Reports injectado' if flag else 'Fallo al injectar')
    ...
    def antiblacklist():
        ...
        replace = bytes.fromhex('43 68 65 61 74 69 6E 67 5F 4D 65 6D 6F 72 79 48 61 63 6B')

        try:
            flag = find_replace_bytes(scan, replace)
            show_notification('Anti blacklist injectado' if flag else 'Fallo al injectar')
